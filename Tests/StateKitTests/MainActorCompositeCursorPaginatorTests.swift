...
@MainActor
@Suite
struct MainActorCompositeCursorPaginatorTests {
    // MARK: - SUT Creation

    private func makeSUT() -> (
        sut: MainActorCursorPaginator<TestPost, TestQuery, CompositeCursor>,
        ...
        let sut = MainActorCursorPaginator<TestPost, TestQuery, CompositeCursor>(remoteLoader: loader.load)

//        Test.trackForMemoryLeaks(sut)
//        Test.trackForMemoryLeaks(loader)
    ...
    func subscribe_emitsUpdatesWhenCacheModified() async throws {
        ...
        let subscriptionTask = Task {
            var iterator = sut.subscribe().makeAsyncIterator()
